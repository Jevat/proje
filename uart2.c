...
unsigned char UART_InChar1(void){
  char letter;
...
void UART_OutChar1(unsigned char data){
...
unsigned long UART_InUDec1(void){
...
void UART_OutUDec1(unsigned long n){
...
//   of unspecified length as an ASCII string
  ...
  UART_OutChar1(n+'0'); /* n is between 0 and 9 */
}
...
// Backspace will remove last digit typed
unsigned long UART_InUHex1(void){
...
void UART_OutUHex1(unsigned long number){
// This function uses recursion to convert the number of
//   unspecified length as an ASCII string
  if(number >= 0x10){
    UART_OutUHex1(number/0x10);
    UART_OutUHex1(number%0x10);
  }
  else{
    if(number < 0xA){
      UART_OutChar1(number+'0');
     }
    else{
      UART_OutChar1((number-0x0A)+'A');
    }
  }
}
...
void UART_InString1(char *bufPt, unsigned short max) {
int length=0;
char character;
  character = UART_InChar1();
  while(character != CR){
